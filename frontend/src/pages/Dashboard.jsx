...
const StatCard = ({ label, value, accent, loading }) => (
  <div
    className={`bg-[var(--bg-surface)] border border-[var(--border)] rounded-[var(--radius)] p-[18px_20px] flex flex-col gap-2 animate-[fadeUp_0.35s_ease_forwards] transition-colors duration-[var(--transition)] hover:border-[var(--border-bright)] ${accent ? "border-[var(--danger-dim)] bg-[var(--danger-glow)]" : ""}`}
  >
    <span className="text-[0.68rem] text-[var(--text-muted)] tracking-[0.08em] uppercase">
      {label}
    </span>
    {loading ? (
      <span
        className="animate-pulse bg-[var(--bg-overlay)]"
        style={{ height: 36, width: 60, borderRadius: 4 }}
      />
    ) : (
      <span className="font-[var(--font-display)] text-[2rem] font-extrabold text-[var(--text-primary)] leading-none">
        {value ?? "—"}
      </span>
    )}
  </div>
    ...
    <div className="p-[20px_16px] md:p-[36px_40px] max-w-[1200px] animate-[fadeUp_0.35s_ease_forwards]">
      ...
      {activeJobs.length > 0 && (
        <section className="mb-10">
          <h2 className="font-[var(--font-display)] text-[0.85rem] font-bold tracking-[0.08em] uppercase text-[var(--text-secondary)] flex items-center gap-2 mb-4">
            <span className="inline-block w-[7px] h-[7px] rounded-full bg-[var(--info)] animate-[pulseDot_1.2s_ease_infinite]" />
            Processing now
          </h2>
          <div className="flex flex-col gap-[10px] mb-2">
            {activeJobs.map(([videoId, job]) => (
              <div
                key={videoId}
                className="bg-[var(--bg-surface)] border border-[var(--border)] border-l-[3px] border-l-[var(--info)] rounded-[var(--radius)] p-[14px_16px] flex flex-col gap-[10px] animate-[fadeUp_0.3s_ease_forwards]"
              >
                <div className="flex items-center justify-between">
                  <span className="text-[0.72rem] text-[var(--text-muted)] tracking-widest">
                    {videoId.slice(-8)}
                  </span>
      ...
      <section className="mb-10">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-[var(--font-display)] text-[0.85rem] font-bold tracking-[0.08em] uppercase text-[var(--text-secondary)] flex items-center gap-2">
            Recent videos
          </h2>
          <Link
            to="/library"
            className="text-[0.75rem] text-[var(--accent)] tracking-wider transition-opacity hover:opacity-70"
          >
            View all →
          </Link>
        </div>
        ...
        {loading ? (
          <div className="grid grid-cols-[repeat(auto-fill,minmax(260px,1fr))] gap-3">
            {[...Array(6)].map((_, i) => (
              <div
                key={i}
                className="animate-pulse bg-[var(--bg-overlay)] rounded-lg h-[110px]"
              />
            ))}
          </div>
        ) : recent.length === 0 ? (
            ...
            {recent.map((v) => {
              ...
              return (
                <Link
                  key={v._id}
                  to={`/library/${v._id}`}
                  className="block bg-[var(--bg-surface)] border border-[var(--border)] rounded-[var(--radius)] p-4 transition-all duration-[var(--transition)] text-none animate-[fadeUp_0.35s_ease_forwards] hover:border-[var(--border-bright)] hover:bg-[var(--bg-raised)]"
                >
                  ...
                  (displayStatus === "processing" ||
                    ...
                    <div className="flex items-center gap-[10px] flex-wrap text-[0.7rem] text-[var(--text-muted)]">

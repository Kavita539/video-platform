  ...
  return (
    <div className="p-[20px_16px] md:p-[36px_40px] max-w-[1200px] animate-[fadeUp_0.35s_ease_forwards]">
      ...
      <div className="flex gap-[10px] mb-6 flex-wrap">
        <input
          className="flex-1 min-w-[200px] bg-surface border border-border rounded-[var(--radius-sm)] text-primary font-[var(--font-mono)] text-[0.82rem] p-[9px_14px] outline-none transition-all duration-[var(--transition)] focus:border-accent focus:shadow-[0_0_0_2px_var(--accent-glow)] placeholder:text-muted"
          placeholder="Search titles…"
          value={search}
          onChange={(e) => setSearch(e.target.value)}
        />
        <select
          className="bg-surface border border-border rounded-[var(--radius-sm)] text-secondary font-[var(--font-mono)] text-[0.78rem] p-[9px_12px] outline-none cursor-pointer transition-colors duration-[var(--transition)] focus:border-accent"
          value={status}
      ...
      {loading ? (
        <div className="flex justify-center p-[80px_0] text-accent">
          <Spinner size="lg" />
        </div>
      ) : videos.length === 0 ? (
        <Empty
          ...
          action={
            <Link
              to="/upload"
              className="inline-flex items-center justify-center gap-2 border border-border-bright rounded-[var(--radius-sm)] font-[var(--font-mono)] font-medium tracking-wider uppercase cursor-pointer transition-all duration-[var(--transition)] bg-transparent text-primary hover:border-accent hover:text-accent p-[10px_20px] text-[0.78rem]"
            >
              Upload video
            </Link>
          }
        ...
        <>
          ...
          <div className="grid grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-3 mb-8">
            {videos.map((v) => {
              ...
              return (
                <div
                  key={v._id}
                  className={`group relative bg-surface border rounded-[var(--radius)] transition-all duration-[var(--transition)] animate-[fadeUp_0.35s_ease_forwards] overflow-hidden ${isFlagged ? "border-[var(--danger-dim)] hover:border-danger" : "border-border hover:border-border-bright"} hover:bg-raised`}
                >
                  <Link
                    to={`/library/${v._id}`}
                  ...
                  >
                    ...
                    {liveJob && displayStatus === "processing" && (
                      <div className="h-[3px] bg-overlay rounded-[2px] mb-[10px] overflow-hidden">
                        <div
                          className="h-full bg-info rounded-[2px] transition-all duration-[0.4s] ease-out"
                          style={{ width: `${liveJob.progress || 0}%` }}
                        />
                      </div>
                    )}

                    <div className="flex gap-[10px] flex-wrap text-[0.68rem] text-muted mb-2 font-mono">
                    ...
                    {v.tags?.length > 0 && (
                      <div className="flex gap-[6px] flex-wrap">
                        {v.tags.slice(0, 3).map((t) => (
                          <span
                            key={t}
                            className="text-[0.65rem] bg-overlay text-muted p-[2px_7px] rounded-[2px] tracking-wider"
                          >
                            {t}
                          </span>
                        ))}
                  ...
                  <button
                    className="absolute top-[10px] right-[10px] bg-transparent border-none text-muted text-[0.75rem] w-[22px] h-[22px] rounded-[var(--radius-sm)] flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-[var(--transition)] hover:text-danger hover:bg-[var(--danger-glow)]"
          ...
          {pages > 1 && (
            <div className="flex items-center justify-center gap-4 p-[20px_0]">
              ...
              <span className="text-[0.75rem] text-muted font-mono">
                Page {page} / {pages}
              </span>
              <Button
                variant="ghost"

  ...
  if (phase === "processing" || phase === "done") {
    return (
      <div className="p-[20px_16px] md:p-[36px_40px] max-w-[1200px] animate-[fadeUp_0.35s_ease_forwards]">
        ...
        <div className="max-w-[520px] bg-[var(--bg-surface)] border border-[var(--border)] rounded-[var(--radius-lg)] p-8 flex flex-col gap-7 animate-[fadeUp_0.35s_ease_forwards]">
          <div className="flex flex-col gap-3">
            {[
              ...
              return (
                <div
                  key={s.key}
                  className={`flex items-center gap-3 text-[0.80rem] transition-colors duration-[var(--transition)] ${isDone ? "text-[var(--text-secondary)]" : isActive ? "text-[var(--text-primary)]" : "text-[var(--text-muted)]"}`}
                >
                  <span
                    className={`w-2 h-2 rounded-full flex-shrink-0 transition-all duration-[var(--transition)] ${isDone ? "bg-[var(--success)]" : isActive ? "bg-[var(--info)] shadow-[0_0_8px_var(--info)] animate-[pulseDot_1s_ease_infinite]" : "bg-[var(--border-bright)]"}`}
                  />
                  <span>{s.label}</span>
                </div>
              );
              ...
              {liveJob.status === "completed" ? (
                <>
                  <div className="flex items-start gap-3.5 p-[14px_16px] rounded-[var(--radius)] text-[0.82rem] bg-[rgba(63,207,142,0.08)] border border-[rgba(63,207,142,0.2)]">
                    <span className="text-[1.2rem] flex-shrink-0 leading-[1.4] text-[var(--success)]">
                      ✓
                    </span>
                    <div>
                      ...
                      <p className="text-[var(--text-muted)] flex items-center gap-1.5 flex-wrap">
                ...
                <div className="flex items-start gap-3.5 p-[14px_16px] rounded-[var(--radius)] text-[0.82rem] bg-[var(--danger-glow)] border border-[var(--danger-dim)]">
                  <span className="text-[1.2rem] flex-shrink-0 leading-[1.4] text-[var(--danger)]">
                    ✕
                  </span>
                  <div>
  ...
  return (
    <div className="p-[20px_16px] md:p-[36px_40px] max-w-[1200px] animate-[fadeUp_0.35s_ease_forwards]">
      ...
      >
        {/* Drop zone */}
        <div
          className={`group border-2 border-dashed rounded-[var(--radius-lg)] text-center cursor-pointer transition-colors duration-[var(--transition)] bg-[var(--bg-surface)] ${dragging ? "border-[var(--accent)] bg-[var(--accent-glow)]" : "border-[var(--border-bright)] hover:border-[var(--accent)] hover:bg-[var(--accent-glow)]"} ${file ? "border-solid border-[var(--accent)] p-[20px_24px] cursor-default" : "p-[48px_24px]"}`}
          onDragEnter={(e) => {
            e.preventDefault();
            setDragging(true);
          }}
        ...
        >
          ...
          {file ? (
            <div className="flex items-center gap-3.5">
              <span className="text-[1.5rem] text-[var(--accent)] flex-shrink-0">
                ▶
              </span>
              ...
              <button
                type="button"
                className="bg-transparent border-none text-[var(--text-muted)] text-[0.9rem] p-1.5 rounded-[var(--radius-sm)] transition-colors duration-[var(--transition)] hover:text-[var(--danger)] hover:bg-[var(--danger-glow)]"
                onClick={(e) => {
                  e.stopPropagation();
                  setFile(null);
                }}
            ...
            <div className="flex flex-col items-center gap-2.5">
              <span
                className={`text-[2rem] text-[var(--text-muted)] ${dragging ? "text-[var(--accent)]" : ""}`}
              >
                ⬆
              </span>
              <span className="text-[0.82rem] text-[var(--text-secondary)]">
                Drop video here or{" "}
                <u className="text-[var(--accent)] no-underline group-hover:underline">
                  browse
                </u>
              </span>
            </div>
